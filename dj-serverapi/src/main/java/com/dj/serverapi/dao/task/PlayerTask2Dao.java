...
import com.dj.serverapi.dao.task.inf.IPlayerTask2Dao;
...
/**
 * @ClassName: PlayerTask2Dao
 * @Description: 每日任务
 * @author zcq
 * @date 2020年2月19日
 */
@Component
public class PlayerTask2Dao extends BaseCacheDao<PlayerTask2> implements IPlayerTask2Dao, ITaskDao<PlayerTask2> {

	@Override
	public List<? extends ITask> flushLevelTask(long roleID, int level) {
		List<PlayerTask2> list = readDbData(roleID);
		int index = 0;
		if(list == null){
		...
		List<PlayerTask2> lists = Lists.newArrayList();
		...
		for(ConfigTasks config : configList) {
			...
			if(isNeedNew) {
				PlayerTask2 task = new PlayerTask2(roleID);
	...
	public ITask flushTask(ITask task) {
		...
		PlayerTask2 playerTask = new PlayerTask2();
	...
	@Override
	public List<PlayerTask2> readDbData(long roleID) {
		QueryParamMap<String, Object> queryParams = new QueryParamMap<>(1);
		queryParams.put("roleID", roleID);
		List<PlayerTask2> list = selectList(queryParams, roleID, AccessType.DIRECT_DB);
	...
	public List<TaskInfo> toTaskInfos(long roleID, int level) {
		...
		List<PlayerTask2> tasks = cacheLoadAll(roleID);
	...
	public List<TaskInfo> getLastTaskInfo(long roleID, int level) {
		...
		List<PlayerTask2> tasks = cacheLoadAll(roleID);
		if((tasks != null)&&(tasks.size() > 0)){
			for(PlayerTask2 playerTask2 : tasks){
				if((playerTask2.getTaskID() > 0) && (playerTask2.getState() < TaskState.award.getState())) {
					if(playerTask2.getState() == TaskState.noAccept.getState()){
						playerTask2.setState(TaskState.accept.getState());
						cacheUpdate(playerTask2, roleID);
					}
					taskInfos.add(playerTask2.toTaskInfo());
					break;
				}
			}
		}
